...
async function inspect(name) {
  const result = await page.evaluate(() => ({
    ...
    smallTargets: [...document.querySelectorAll("button,a,input,select")]
      ...
      .map((node) => ({
        text: (node.getAttribute("aria-label") || node.textContent || "").trim().slice(0, 30),
...
await page.getByRole("button", { name: /Continue/ }).click();
const electric = page.getByRole("spinbutton", {
  name: "Electric maximum amount",
});
...
const buffer = page.getByLabel("Cushion amount");
