...
DECLARE
  ...
  v_case uuid;
  v_count integer := 0;
  v_context uuid := nullif(current_setting('app.household_id', true), '')::uuid;
BEGIN
  IF p_household_id IS NULL OR v_context IS DISTINCT FROM p_household_id THEN
    RAISE EXCEPTION 'tenant context required';
  END IF;

  WITH latest AS (
    SELECT DISTINCT ON (t.household_id,t.account_id,t.source_kind,t.source_record_id)
      ...
      t.occurred_on,t.status,t.direction
    FROM financial_transactions t
    WHERE t.household_id=p_household_id
    ORDER BY t.household_id,t.account_id,t.source_kind,t.source_record_id,t.revision DESC
  ), candidates AS (
    SELECT 'duplicate:'||encode(digest(
      least(a.source_kind||':'||a.source_record_id,b.source_kind||':'||b.source_record_id)||'|'||
      greatest(a.source_kind||':'||a.source_record_id,b.source_kind||':'||b.source_record_id),
      'sha256'),'hex') AS detection_key
    FROM latest a JOIN latest b
      ON b.household_id=a.household_id AND b.account_id=a.account_id
    ...
    WHERE a.status='posted' AND b.status='posted'
      AND a.direction='debit' AND b.direction='debit'
  ...
  SET status='expired',version=c.version+1,updated_at=now()
  WHERE c.household_id=p_household_id
    AND c.case_type='possible_duplicate'
    AND c.detection_key IS NOT NULL
    AND c.status IN ('open','decided','awaiting_verification')
    AND NOT EXISTS (SELECT 1 FROM candidates x WHERE x.detection_key=c.detection_key);
  ...
  FOR r IN
    ...
    SELECT a.household_id,
      ...
      b.source_kind AS second_provenance,a.account_id,acct.name AS account_name,
      'duplicate:'||encode(digest(
        least(a.source_kind||':'||a.source_record_id,b.source_kind||':'||b.source_record_id)||'|'||
        greatest(a.source_kind||':'||a.source_record_id,b.source_kind||':'||b.source_record_id),
        'sha256'),'hex') AS detection_key
    FROM latest a JOIN latest b
  ...
  LOOP
    ...
    ON CONFLICT(household_id,detection_key) WHERE detection_key IS NOT NULL DO NOTHING
    ...
    INSERT INTO case_evidence(
      ...
      (r.household_id,v_case,'transaction','financial_transaction',r.first_id,'First matching charge',r.first_merchant,r.first_amount_minor,r.first_currency,r.first_occurred_on,r.account_id,r.account_name,r.first_status,r.first_provenance),
      (r.household_id,v_case,'transaction','financial_transaction',r.second_id,'Second matching charge',r.second_merchant,r.second_amount_minor,r.second_currency,r.second_occurred_on,r.account_id,r.account_name,r.second_status,r.second_provenance);
    v_count:=v_count+1;
  END LOOP;
  RETURN v_count;
END $$;
...
GRANT UPDATE (status, version, updated_at) ON exception_cases TO budgefi_app;

CREATE OR REPLACE FUNCTION prepare_account_deletion_membership()
RETURNS TABLE(active_household_count integer, successor_user_id uuid)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
...
BEGIN
  ...
  WHERE m.household_id=v_household AND m.user_id=v_user AND m.revoked_at IS NULL;
  IF v_role IS DISTINCT FROM 'owner' THEN RAISE EXCEPTION 'owner role required'; END IF;

  -- A user scope without a household can count only that user's memberships.
  PERFORM set_config('app.household_id', '', true);
  SELECT count(*)::integer INTO v_count
  FROM household_memberships m
  WHERE m.user_id=v_user AND m.revoked_at IS NULL;

  -- A system household scope can choose a successor without exposing the
  -- household roster to the application role.
  ...
  IF v_successor IS NOT NULL THEN
    ...
    SET role='owner'
    WHERE household_id=v_household AND user_id=v_successor AND revoked_at IS NULL;
  END IF;

  PERFORM set_config('app.household_id', v_household::text, true);
  PERFORM set_config('app.user_id', v_user::text, true);
  RETURN QUERY SELECT v_count,v_successor;
END $$;

REVOKE ALL ON FUNCTION prepare_account_deletion_membership() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION prepare_account_deletion_membership() TO budgefi_app;
