...
export function CommitmentEditor({item,compact=false}:{item:PlanCommitment;compact?:boolean}) {
  ...
  const updateCalibration=(remove=false):PlanCalibrationData=>{
    if(fixed){
      ...
      return {...state.calibration,[fixed.amount]:remove?0:amount,[fixed.date]:remove?"":dueDate,editedCommitments:edited};
    }
  ...
  return <Sheet open={open} onOpenChange={next=>{setOpen(next);if(next)resetDraft()}}>
    ...
    <SheetContent side="bottom" className="mx-auto max-w-[430px] rounded-t-[28px]">
      ...
      {!fixed&&<><label className="mt-5 block text-sm font-semibold" htmlFor={`commitment-name-${item.id}`}>Name</label><input id={`commitment-name-${item.id}`} value={name} onChange={event=>setName(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-rule bg-white px-3 text-base font-semibold outline-none focus:ring-2 focus:ring-pencil"/></>}
